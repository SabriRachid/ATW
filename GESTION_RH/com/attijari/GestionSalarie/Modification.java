...
public class Modification extends ConnexionBDD{
	...
	public void remplir (){
		...
		try{
			...
			String req = "select  Nom, Prenom, dateNaissance, Adresse, Email, Nationalite, DateEmbaucheGroupe, DateEmbaucheFiliale,"
					...
					+ " NombrJoursDispo,reliquatAnneEnCours ,loginVdoc, sexeMasculin, FilialeIdFiliale, lieuNaissance, DirectionIdDirection, RIB, supMatricule,profilUser,Superieur2, Statut from "
			...
			while (rs.next())
			{
				...
				getWorkflowInstance().setValue("P_GS_Managers", rs.getString(29));
				boolean statut = rs.getBoolean(30);
				if(statut == true){
	...
	{
		//String chemin = getWorkflowInstance().getCatalog().getConfiguration().getStringUserProperty("Chemin");
		//	getResourceController().alert("le chemin est "+chemin);
//		remplir();
//		Privilege();  
