...
public class CongeProviderExceptionnelle extends BaseViewProvider implements ICollectionModelViewProvider {
	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	protected static final Logger log = Logger.getLogger(CongeProviderExceptionnelle.class);
	 ...
	 * @param view
	 */
	public CongeProviderExceptionnelle(INavigateContext context, CtlAbstractView view) {
		super(context, view);
	}
	...
	public List<ViewModelItem> getModelItems() {
		...
		try {
			...
			IWorkflowInstance workflowInstance = ApplicationDataProvider.workflowInstance;//ApplicationDataProvider.workflowInstance;
	...
	ArrayList<Conge> getConges(IWorkflowInstance workflowInstance) {
		...
		try {
			...
			//List<IUser> users = (List<IUser>) workflowInstance.getValue("PERSONNEL");
			IUser user = getWorkflowModule().getLoggedOnUser();
			String login = user.getLogin();//users.size()>0 ? users.get(0).getLogin() : "";
			...
			String query = "SELECT distinct n.CodeVdocDemandeConge,n.IdConge,n.TypeConge,n.DateDeb,n.DateFin,s.IdConge,s.TypeConge,s.DateDeb,s.DateFin  "
					+ ",s.CodeVdocDemandeConge FROM Conge n,Conge s where (n.DateDeb<=SYSDATETIME() ) and DATEDIFF(DAY,n.DateFin,SYSDATETIME())<31 and   n.Personnelmatricule=? and (n.EtatConge not like 'congé annulé' and n.EtatConge not like 'congé modifié') and n.CodeVdocDemandeConge not like 'GCC%' and s.Personnelmatricule=n.Personnelmatricule and (n.DateFin = s.DateDeb and n.IdConge <> s.IdConge) and s.CodeVdocDemandeConge = n.CodeVdocDemandeConge"
					+ " union "
					...
					+ " ,s.CodeVdocDemandeConge FROM Conge n,Conge s where (n.DateDeb<=SYSDATETIME() ) and DATEDIFF(DAY,n.DateFin,SYSDATETIME())<31 and  s.TypeConge=n.TypeConge and (n.EtatConge not like 'congé annulé' and n.EtatConge not like 'congé modifié') and n.CodeVdocDemandeConge not like 'GCC%' and n.Personnelmatricule=? and s.Personnelmatricule=n.Personnelmatricule and  ( n.IdConge = s.IdConge and "
					+ "(select COUNT(nn.CodeVdocDemandeConge) from Conge nn where nn.CodeVdocDemandeConge= n.CodeVdocDemandeConge )=1) order by n.DateFin desc ";
