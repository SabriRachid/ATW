...
public class DemandeFacturationV1 extends ConnexionBDD{

    /**---------------------------------------*
     * @Created_on 08/12/2016 
     * @Updated_on 02/01/2018
     * @author r.sabri
     * @version 4.0
     *----------------------------------------*/
    ...
    public void onPropertyChanged(IProperty property) {
        ...
        if(property.getName().equals("PF_Dossier"))
        {
            try {
                ...
                if (Dossier!=null)
                {
                    ...
                    Budget();    			        // BUDGET DÉFINITIVE
                    if (FactureExiste(Dossier)) 
                        LeReste(Dossier);   		// LE RESTE DES PAIEMENT
                    ...
                    if ((Object) getWorkflowInstance().getValue("PF_LasteReste")!=null)
                    {
                        Object TotPaiement = (Object) getWorkflowInstance().getValue("PF_LasteReste");
                        DtotalBudget = Double.parseDouble(TotPaiement.toString());
                    }
                    //getWorkflowInstance().setValue("PF_LasteReste", (double)(Budget-Montant)- DtotalBudget);
                    getWorkflowInstance().setValue("PF_LasteReste", DtotalBudget);
                }else{
                    ClearForm();			
                    getResourceController().showBodyBlock("FragMandat", false);
                    getResourceController().showBodyBlock("FragHistoriqueFactures", false);
        ...
        {
            if (!(getWorkflowInstance().getValue("PF_MontantStr").equals("")))
            {
                if (!isNumeric((String) getWorkflowInstance().getValue("PF_MontantStr")))
                {
                    getResourceController().alert("LE MONTANT NON VALIDE");
                }else {
                    Object MT = (Object) getWorkflowInstance().getValue("PF_MontantStr");
                    double Montant = Double.parseDouble(MT.toString().replaceAll(" ", ""));  
                    ...
                    double DtotalBudget=0d;
                    if ((Object) getWorkflowInstance().getValue("PF_Tab_PaiementTotal")!=null)
                    {
                        Object TotPaiement = (Object) getWorkflowInstance().getValue("PF_Tab_PaiementTotal");
                        DtotalBudget = Double.parseDouble(TotPaiement.toString());
                    }
                    if (Montant<=Budget && Montant>0)
                    {
                        String Dossier = (String) getWorkflowInstance().getValue("PF_Dossier");
                        if (FactureExiste(Dossier))
                        {
                            ...
                            getWorkflowInstance().setValue("PF_LasteReste", Math.abs(Budget-Montant)); 
                            getWorkflowInstance().setValue("PF_LasteResteSM", ConnexionBDD.SeparateurMilliers(Math.abs(Budget-Montant)));
                        }
                        DernierPaiement();     // CE MÉTHODE INDIQUE SI LA FACTURE EST TOTALEMENT PAYEÉ OU NON
                    }else if(Montant>Budget){
                        getResourceController().alert("LE MONTANT EST SUPÉRIEUR AU BUDGET");
                        getWorkflowInstance().setValue("PF_LasteReste", 0.0);
                        getWorkflowInstance().setValue("PF_LasteResteSM", "0.0");
                        getWorkflowInstance().setValue("PF_MontantStr", "0.0");
                    }else if(Montant==0) {
                        //getResourceController().alert("LE MONTANT EST ÉGAL À ZÉRO");
                    }else {
                        getResourceController().alert("LE MONTANT EST INFÉRIEUR À ZÉRO");
                        getWorkflowInstance().setValue("PF_LasteReste",DtotalBudget);        
                        getWorkflowInstance().setValue("PF_LasteResteSM",ConnexionBDD.SeparateurMilliers(DtotalBudget));
                        getWorkflowInstance().setValue("PF_MontantStr", "0.0");
                    }
                }
            }
    ...
    public boolean onBeforeSubmit(IAction action) {
        ...
        {
            // RETOURNE "TRUE" SI LES CONTRÔLE DE VALIDATION EST VALIDE ET "FALSE" POUR LE CONTRAIRE
            if (getWorkflowInstance().getValue("PF_LasteReste")!=null)
            {
                Object Rest = (Object) getWorkflowInstance().getValue("PF_LasteReste");
                Double LeReste = Double.parseDouble(Rest.toString());
                if (LeReste<0)
                {
                    ok = false;
                    getResourceController().alert("LE MONTANT EST SUPÉRIEUR AU RESTE DE LA FACTURE !");
                }else{
                    Object MONT = (Object) getWorkflowInstance().getValue("PF_MontantStr");  // MANTANT SAISIE
                    Object BUDG = (Object) getWorkflowInstance().getValue("PF_Budget");      // BUDGET DÉFINITIVE
                    Double Montant = Double.parseDouble(MONT.toString().replaceAll(" ", ""));
                    Double Budget = Double.parseDouble(BUDG.toString()) ;		
                    if (Montant<=Budget && Montant>0)                    
                    {
                        ok = true;
                    }else{
                        ok = false;
                        getResourceController().alert("LE MONTANT EST SUPÉRIEUR AU BUDGET OU ÉGALE À 0 !");
                    }
                }
            }else{
                ok = false;
    ...
    // ----------------------------------------------------------------------------------------------------
    // RÉCUPERER CLIENT / METHODE INUTILISABLE - non utiliser
    // ----------------------------------------------------------------------------------------------------
    public String Client(String Dossier) {
        cnx=null;
        st=null;
        rs=null;
        String customer = "";
        try {
            // DÉFINITION DE LA CONNEXION
            cnx=getConnectionVDoc("Ref_Attijari").getConnection();
            String query ="select Client from Dossiers where RefDossier=?";
            st = cnx.prepareStatement(query);
            st.setString(1, Dossier);
            rs = st.executeQuery();
            while(rs.next())
            ...
            e.printStackTrace();
            log.error("Erreur dans la methode Client: " + e.getClass() + " - " + message);
        }finally {
            // LIBÉRER RESSOURCES DE LA MÉMOIRE.
            ConnexionBDD.close(cnx, st, rs);
        }
    ...
    // ----------------------------------------------------------------------------------------------------
    // MÉTHODE DE CALCULE DE RESTE DES FACTURE PAYANTS
    // ----------------------------------------------------------------------------------------------------
    public void LeReste(String Dossier)
    {
        cnx=null;
        st=null;
        rs=null;
        try{
            Object BDget = (Object) getWorkflowInstance().getValue("PF_Budget");
            double Budget = Double.parseDouble(BDget.toString()); 
            cnx=getConnectionVDoc("Ref_Attijari").getConnection();
            String Requette = "select (?-sum(Montant_Fact)) as 'LeReste' from Factures where Dossier=?";
            st = cnx.prepareStatement(Requette);
            st.setDouble(1, Budget);
            st.setString(2, Dossier);
            rs = st.executeQuery();
            while(rs.next()){
                getWorkflowInstance().setValue("PF_LasteReste", rs.getDouble("LeReste")); // Hidden value
                getWorkflowInstance().setValue("PF_LasteResteSM", ConnexionBDD.SeparateurMilliers(Math.abs(rs.getDouble("LeReste"))));
            }	
        }catch (Exception e) {
            String message = e.getMessage();
            if (message == null)
            ...
            e.printStackTrace();
            log.error("Erreur dans la methode LeReste : " + e.getClass() + " - " + message);
        }finally {
            // LIBÉRER RESSOURCES DE LA MÉMOIRE.
            ConnexionBDD.close(cnx, st, rs);
        }
